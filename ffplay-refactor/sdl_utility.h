...
int realloc_texture(SDL_Renderer** renderer, SDL_Texture **texture, Uint32 new_format, int new_width, int new_height,
...
int upload_texture(SDL_Renderer** renderer, SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx);
void set_sdl_yuv_conversion_mode(AVFrame *frame);

/* 设置窗口大小并以指定颜色填充窗口 */
inline void fill_rectangle(int x, int y, int w, int h, SDL_Renderer* renderer)
{
	SDL_Rect rect;
	rect.x = x;
	rect.y = y;
