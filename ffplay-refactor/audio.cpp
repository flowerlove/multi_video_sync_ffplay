...
int audio_decode_frame(MediaState * ms)
{
	...
	do {
		...
		{
			if ((av_gettime_relative() - ms->audio_callback_time) > 1000000LL * ms->audio_hw_buf_size / ms->audio_tgt.bytes_per_sec / 2)
				return -1;
			av_usleep(1000);
		}
#endif
...
void sdl_audio_callback(void * opaque, Uint8 * stream, int len)
{
	...
	ms->audio_callback_time = av_gettime_relative();//获取当前系统时间
