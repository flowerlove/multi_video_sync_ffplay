...
int is_pkt_in_play_range(MediaState*ms, AVFormatContext * ic, AVPacket * pkt)
{
	...
	double ic_ts = stream_ts_s - get_ic_start_time(ms, ic);

	return ic_ts <= ((double)ms->duration / 1000000);
}
...
void stream_component_close(MediaState * ms, int stream_index)
{
	...
	//	SDL_CloseAudioDevice(ms->audio_dev);
...
int read_thread(void * arg)
{
	...
	if (ms->seek_by_bytes < 0)
		ms->seek_by_bytes = !!(fmt_ctx->iformat->flags & AVFMT_TS_DISCONT) && strcmp("ogg", fmt_ctx->iformat->name);
	...
	if (st_index[AVMEDIA_TYPE_VIDEO] >= 0)
	{
		...
		if (codecpar->width)
		{
			...
			ms->default_width = codecpar->width;
			ms->default_height = codecpar->height;
		}
	}
	...
	while (true)
	{
		...
		if (ms->seek_req) 
		{
			...
			else
			{
				if (ms->audio_stream >= 0)
				{
					...
					ms->audio_pq.packet_queue_put(&MediaState::flush_pkt);
				}
				if (ms->subtitle_stream >= 0)
				{
					ms->subtitle_pq.packet_queue_flush();
					ms->subtitle_pq.packet_queue_put(&MediaState::flush_pkt);
				}
			...
			(!ms->video_st || (ms->viddec.is_finished() == *ms->video_pq.get_serial() && ms->pict_fq.frame_queue_nb_remaining() == 0)))
		{
			...
			if (allow_loop())
			{
				stream_seek(ms, ms->start_time != AV_NOPTS_VALUE ? ms->start_time : 0, 0, 0);
			}
		...
		pkt_in_play_range = is_pkt_in_play_range(ms, fmt_ctx, pkt);
...
bool stream_open(MediaState* ms, const char * filename)
{
	...
	ms->refresh_tid = SDL_CreateThread(refresh_thread, "refresh_thread", ms);
	if (!ms->refresh_tid)
	{
		av_log(NULL, AV_LOG_FATAL, "SDL_CreateThread(): %s\n", SDL_GetError());
		stream_close(ms);
